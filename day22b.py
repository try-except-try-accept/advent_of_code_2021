...
TEST_DELIM = "---"
FILE_DELIM = "\n"
TESTS = """on x=-5..47,y=-31..22,z=-19..33
...
def solve(data):
    ...
    for ins in instructions:
        mode, x_from, x_to, y_from, y_to, z_from, z_to = ins
        print("Processing", ins)
        new = set([(i,j,k) for i in range(x_from, x_to+1) for j in range(y_from, y_to+1) for k in range(z_from, z_to+1)])

        if mode:
            cubes = cubes.union(new)
        else:
            cubes = cubes.difference(new)
